...
public class BluetoothChat extends AppCompatActivity{
    ...
    private MainActivity.ChatService mChatService = null;
    @Override
    ...
    public synchronized void onResume() {  //synchronized：同步方法实现排队调用
        super.onResume();
        if (mChatService != null) {
            if (mChatService.getState() == MainActivity.ChatService.STATE_NONE) {
                mChatService.start();
            }
        }
    }
    private void setupChat() {
        ...
        mSendButton.setOnClickListener(new DialogInterface.OnClickListener() {
        ...
        //创建服务对象
        mChatService = new MainActivity.ChatService(this, mHandler);
    ...
    private void sendMessage(String message) {
        if (mChatService.getState() != MainActivity.ChatService.STATE_CONNECTED) {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case MESSAGE_STATE_CHANGE:
                    switch (msg.arg1) {
                        case MainActivity.ChatService.STATE_CONNECTED:
                            mTitle.setText(R.string.title_connected_to);
                            mTitle.append(mConnectedDeviceName);
                            mConversationArrayAdapter.clear();
                            break;
                        case MainActivity.ChatService.STATE_CONNECTING:
                            mTitle.setText(R.string.title_connecting);
                            break;
                        case MainActivity.ChatService.STATE_LISTEN:
                        case MainActivity.ChatService.STATE_NONE:
